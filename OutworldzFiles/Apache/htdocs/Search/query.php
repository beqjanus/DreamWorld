...
function dir_places_query($method_name, $params, $app_data)
{
    ...
    while ($row = $query->fetch(PDO::FETCH_ASSOC))
    {
        ...
        @$data[] = array(
                ...
                "name" => $row["gateway"]. ' ' . $row["regioname"]." ".$row["parcelname"],
                "for_sale" => "False",
                "auction" => "False",
...
xmlrpc_server_register_method($xmlrpc_server, "init_SearchDataUpdate",        "init_SearchDataUpdate");
function init_SearchDataUpdate($method_name, $params, $app_data)
{
   $data = array();
  $response_xml = xmlrpc_encode(array(
            'success'      => True,
...
// flags:50331680
    ...
    flog("day = " . $day);
    flog("Category = " . $category);
    flog("Search text = " . $search_text);
    ...
    if ($day == "u")    //Searching for current or ongoing events?
    {
      ...
      // fkb   $terms[] = "dateUTC+duration*60 >= " . $now;
    }
    else
    {
      ...
      //fkb   $terms[] = "(dateUTC+duration*60 >= $now AND dateUTC < $then)";
    }
...
xmlrpc_server_register_method($xmlrpc_server, "server_register",       "server_register");
function server_register($method_name, $params, $app_data)
{
   $response_xml = xmlrpc_encode(array(
            'success'      => True,
            'errorMessage' => ""
